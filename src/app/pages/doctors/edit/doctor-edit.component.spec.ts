...
describe('DoctorEditComponent', () => {
  ...
  it('should navigate to /doctors when onSave is called', () => {
    component.onSave();
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/doctors']);
  });

  it('should navigate to /doctors when onCancel is called', () => {
    component.onCancel();
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/doctors']);
  });

  it('should update form when input value changes', () => {
